...
class ParallelDataCollection:
    def __init__(self, config_file, width=None, height=None, save_state=False, camera_config_detail=False):
        ...
        _drone_type_cfg = self.config.get('drone_config', {}).get('drone_type', 'DJIS900')
        self.drone_type = _drone_type_cfg[0] if isinstance(_drone_type_cfg, list) else _drone_type_cfg
    ...
    def _setup_run_parameters(self, config_file):
        ...
        drone_type_config = self.config['drone_config'].get('drone_type', 'DJIS900')
        if isinstance(drone_type_config, list):
            self.drone_types = drone_type_config if drone_type_config else ['DJIS900']
        else:
            self.drone_types = [drone_type_config]
        # Initialize with the first drone type
        drone_type = self.drone_types[0]
        ...
        # Load camera config folder if specified (multiple camera configs for switching)
        self.camera_config_list = []
        self.camera_config_switch_interval = 0
        cam_folder = self.config.get('camera_config_folder')
        if cam_folder:
            self.camera_config_list = self._load_camera_configs_from_folder(cam_folder)
            if self.camera_config_list:
                # Seed camera_config with the first file so initial spawn works
                first_name, first_cfg = self.camera_config_list[0]
                self.config['camera_config'] = first_cfg
                num_configs = len(self.camera_config_list)
                self.camera_config_switch_interval = max(1, self.max_images // num_configs)
                print(
                    f"[INFO] Camera config rotation enabled: {num_configs} configs, "
                    f"switching every {self.camera_config_switch_interval} frames"
                )
    ...
    def _load_camera_configs_from_folder(self, folder_path):
        """Load all camera config JSON files from a folder, sorted by filename.
        
        The files are bare JSON fragments (key-value pairs without wrapping braces),
        so they are wrapped in {} before parsing.
        """
        if not os.path.isdir(folder_path):
            print(f"[ERROR] Camera config folder not found: {folder_path}")
            return []
        ...
        if not json_files:
            print(f"[WARNING] No JSON files found in camera config folder: {folder_path}")
            return []
        ...
        for filename in json_files:
            ...
            try:
                with open(filepath, 'r') as f:
                    raw = f.read().strip()
                # Wrap in braces if the file is a bare fragment
                if not raw.startswith('{'):
                    raw = '{' + raw + '}'
                config = json.loads(raw)
                configs.append((filename, config))
                print(f"[INFO] Loaded camera config: {filename} ({len(config)} cameras)")
            except Exception as e:
                print(f"[WARNING] Failed to load camera config {filename}: {e}")

        print(f"[INFO] Loaded {len(configs)} camera config files from {folder_path}")
        return configs

    def _reconfigure_cameras(self, camera_config):
        """Reconfigure existing cameras with a new camera config (no spawning).
        
        Updates self.config['camera_config'] and re-attaches/re-positions
        each camera using the new configuration.
        """
        self.config['camera_config'] = camera_config

        for camera_id, cam_cfg in camera_config.items():
            camera_num = int(camera_id.replace('camera', ''))
            camera_type = cam_cfg.get('type', '').lower()
            attached_to_drone = cam_cfg.get('attached_to_drone', False)
            is_attached = (
                camera_type in {'attachdrone', 'attachcamera', 'attach_cam', 'attach'}
                or attached_to_drone
            )

            if is_attached:
                self._attach_camera_to_drone(camera_num, cam_cfg)
            else:
                self._setup_static_camera(camera_num, cam_cfg)

        print(f"[INFO] Reconfigured {len(camera_config)} cameras with new config")
    ...
    def capture_frames_parallel(self):
        ...
        # Track which camera config is currently active (for folder-based rotation)
        current_cam_config_idx = 0
        ...
        while self.continue_capture and self.frame_index < self.max_images and (time.time() - start_time) < self.capture_duration:
            ...
            # Switch camera config if using folder-based rotation
            if self.camera_config_list and self.camera_config_switch_interval > 0:
                target_idx = min(
                    self.frame_index // self.camera_config_switch_interval,
                    len(self.camera_config_list) - 1,
                )
                if target_idx != current_cam_config_idx:
                    current_cam_config_idx = target_idx
                    cfg_name, cfg_data = self.camera_config_list[current_cam_config_idx]
                    print(
                        f"\n[CAM-SWITCH] Frame {self.frame_index}: switching to camera config "
                        f"{current_cam_config_idx + 1}/{len(self.camera_config_list)} ({cfg_name})"
                    )
                    self.unrealcv_client.request('vset /action/game/pause')
                    self._reconfigure_cameras(cfg_data)
                    self.unrealcv_client.request('vset /action/game/resume')
    ...
    def run(self, config_files):
        ...
        for i, config_file in enumerate(config_files):
            ...
            for j, drone_type in enumerate(self.drone_types):
                print(f"\n=== Processing drone type: {drone_type} ({j+1}/{len(self.drone_types)}) ===")
                if j > 0:
                    # Switch to a new drone type: update settings.json and reload the level
                    # so AirSim picks up the new PawnBP on reconnect.
                    self.init_drone_type(drone_type)
                    self.drone_type = drone_type
                    self.drone_blueprint = f"BP_{drone_type}"
                    base_dir = self.config['data_collection']['base_output_dir']
                    self.level_output_dir = os.path.join(base_dir, drone_type, current_level)
                    print(f"Reloading level {current_level} to apply new drone type {drone_type}...")
                    self._change_level(current_level)
                ...
                if not airsim_available:
                    ...
                    break
                # Setup cameras and agent colors for this drone type
                ...
                print(f"Completed all time of day scenarios for {current_level} with drone type {drone_type}.")
            print(f"Completed all drone types for {current_level}.")
...
if __name__ == "__main__":
    ...
    config_files_to_process = [
        ...
        # r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\big-close-drones\config-forest.json",
        # r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\big-close-drones\config-lake.json",
        # r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\big-close-drones\config-river.json",
        # r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\big-close-drones\config-rural.json",
        # r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\big-close-drones\config-city.json",
        
        
        # experiment 7 location dist study
        r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\experiment7-location-dist-study\config-forest.json",
        r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\experiment7-location-dist-study\config-city.json",
        r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\experiment7-location-dist-study\config-lake.json",
        r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\experiment7-location-dist-study\config-river.json",
        r"C:\Users\Josh\Desktop\PhD\Research\SiDG-ATRID\git\SIDG-ATRID\scripts\Data_collection\main\config\Ablation-Studies\experiment7-location-dist-study\config-rural.json",
    ]
